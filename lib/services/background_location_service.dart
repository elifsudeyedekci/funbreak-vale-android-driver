...
class BackgroundLocationService {
  ...
  static Future<void> initialize() async {
    ...
    await service.configure(
      androidConfiguration: AndroidConfiguration(
        onStart: _onStart,
        autoStart: false, // Manuel başlatılacak (yolculuk başlayınca)
        isForegroundMode: true, // Foreground service (bildirim gösterir)
  ...
  static Future<void> _onStart(ServiceInstance service) async {
    ...
    // Servisi durdur komutu
    service.on('stopService').listen((event) {
      print('🛑 Background service durduruluyor...');
      service.stopSelf();
    });
    
    // Aktif yolculuk ID'sini al
    ...
    // Her 5 saniyede bir konum gönder (uygulama kapalı olsa bile!)
    Timer.periodic(const Duration(seconds: 5), (timer) async {
      ...
      // Service'e veri gönder (UI güncellemesi için)
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: json.encode({
          ...
          'timestamp': DateTime.now().toIso8601String(),
          'is_online': 1,
          'source': 'background_service', // Backend'de ayırt etmek için
  ...
  static Future<void> startRideTracking(String rideId) async {
    try {
      ...
      // SharedPreferences'a kaydet (service restart durumunda)
      final prefs = await SharedPreferences.getInstance();
  ...
  /// Aktif yolculuk varsa servisi yeniden başlat (app restart durumunda)
